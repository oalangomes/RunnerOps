...
\t' read -r job_id DETAIL_JOB job_conclusion DETAIL_RUNNER_NAME DETAIL_RUNNER_GROUP DETAIL_RUNNER_LABELS DETAIL_STEP step_conclusion <<< "$row"
}
...
diagnose_waiting_self_hosted() {
  local run_id job_row job_id job_name job_status runner_name runner_group labels
  local runners row runner_id candidate_name candidate_status candidate_busy candidate_labels
...
\t' read -r job_id job_name job_status runner_name runner_group labels; do
      [[ -n "${job_id:-}" ]] || continue
...
\t' read -r runner_id candidate_name candidate_status candidate_busy candidate_labels; do
...
\t' read -r run_id workflow status conclusion url attempt; do
    [[ -n "${run_id:-}" ]] || continue
    run_count=$((run_count + 1))
    terminal_signature+="${run_id}:${attempt}:${status}:${conclusion}|"
